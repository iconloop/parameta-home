...
.ht2 .faq-row{ padding:var(--space-24) 0 }   /* 좌우는 카드 인셋만 */
.ht2 .faq-item.open .faq-row{ padding:var(--space-24) 0 }   /* 열림 상태도 동일 (공유 2.25rem 오버라이드 무력화) */
...
.ht2-subwrap .ht2-subin{ overflow:hidden }   /* 좌우는 카드 인셋만 */
...
.ht2-rows li:last-child{ padding-bottom:var(--space-24) }   /* 마지막 행 아래 = 기준 단위 */
.ht2-rows li{ display:flex; gap:var(--space-16);   /* 디바이더 없이 패딩으로만 그룹핑, 갭은 연도와 동일 16 */
  padding:var(--space-12) 0; font-size:var(--text-body); font-weight:var(--w-title); color:var(--c-body); line-height:var(--lh-body) }   /* 본문 한 단계 밝게(c-body) */
